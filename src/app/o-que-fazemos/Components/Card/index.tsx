...
export default function Card({ card }: Card) {
  return (
    <div className="w-[335px] flex flex-col rounded-xl shadow-lg xs:w-full">
      <div className="w-[335px] h-[220px] xs:w-full ">
        <div style={{ backgroundImage: `url(${card.img})` }} className="w-full h-full bg-cover" />
      </div>
      <div className="w-full text-center text-h5 font-Mulish font-bold px-7 pb-4 pt-5">{card.title}</div>
      <div className="w-full text-pm px-7 pb-6 text-start">{card.desc}</div>
